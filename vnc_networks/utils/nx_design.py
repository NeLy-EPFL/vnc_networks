...
def draw_graph(
    ...
    pos_nx_method: typing.Callable[[nx.DiGraph], Mapping] = nx.circular_layout,
...
def draw_graph(
    ...
    node_labels = {k: labels[k] if k in labels.keys() else k for k in G.nodes}
...
def add_edge_legend(
    ...
    lines = []
    edges_weight_list = sorted([np.abs(w) for w in weights if w != 0], reverse=True)
    ...
    for n_, dict_ in nodes_:
        if "node_color" in dict_.keys():
            ...
            if "color" in dict_.keys():
                ...
                node_colors.append(params.NEURON_CLASSES[dict_["node_class"]]["color"])
            else:
...
def get_ring_pos_sorted_alphabetically(
    ...
    raw_pos = nx.circular_layout(subgraph, scale=radius, center=center)  # type: ignore NetworkX isn't typed to accept floats

    def get_node_label(node):
        if ("node_label" not in subgraph.nodes[node].keys()) or (
            subgraph.nodes[node]["node_label"] is None
        ):
            return ""
...
def draw_graph_concentric_circles(
    ...
    output: typing.Literal["graph"] = "graph",
) -> matplotlib.axes.Axes: ...
@typing.overload
def draw_graph_concentric_circles(
    graph: nx.DiGraph,
...
def draw_graph_concentric_circles(
    ...
    center: Optional[tuple] = None,
    radius_scaling: float = 1,
    output: typing.Literal["graph", "pos"] = "graph",
) -> matplotlib.axes.Axes | Mapping:
    """
    ...
    outer_ring = [n for n in graph_.nodes if len(list(graph_.successors(n))) == 0]
    ...
    if edge_norm is None:
        try:
            edge_norm = (
                max([np.abs(graph_.edges[e]["weight"]) for e in graph_.edges]) / 5
            )
        except ValueError:
            edge_norm = 1
    assert edge_norm is not None  # needed for type hinting
    widths = [np.abs(graph_.edges[e]["weight"]) / edge_norm for e in graph_.edges]
...
def draw_graph_grouped_by_attribute(
    ...
    for node in graph.nodes():
        if (
            attribute not in graph.nodes[node].keys()
            or graph.nodes[node][attribute] is None
        ):
    ...
    if (
        (center_nodes is None)
        and (center_instance is not None)
        and (center_instance in clusters)
    ):
    ...
    node_boundary_colors = define_node_boundary_colors(graph)  # noqa: F841
    node_labels = {
        n: graph.nodes[n]["node_label"] if "node_label" in graph.nodes[n].keys() else ""
        for n in graph.nodes
    }
    nx.draw(
        graph,
    ...
    if (center_nodes is None) and (center_instance is not None):
        ax.text(
            0,
            0,
            center_instance,
...
def draw_graph_concentric_by_attribute(
    ...
    # count the number of clusters defined
    cluster_list = [
        graph.nodes[node][attribute] for node in target_nodes if node in graph.nodes
    ]
    clusters = list(set(cluster_list))
    clusters.sort()
    if clusters == []:
    ...
    cluster_centers = [
        (
            (outer_radius * params.FIGSIZE[0])
            * np.cos(2 * np.pi * cluster_nb / nb_clusters),
            (outer_radius * params.FIGSIZE[1])
            * np.sin(2 * np.pi * cluster_nb / nb_clusters),
        )
        for cluster_nb in range(nb_clusters)
    ]
    leaf_scaling = 1 / nb_clusters * params.FIGSIZE[0]
    ...
    other_nodes = [
        node
        for node in graph.nodes
        if node not in target_nodes and node not in center_nodes
    ]
    ...
    def weighted_angle(graph, node_to_place, connected_nodes, positions):
        ...
        weights = [
            np.abs(graph.edges[(node_to_place, n)]["weight"]) for n in connected_nodes
        ]
...
def draw_graph_in_out_center_circle(
    graph: nx.DiGraph,
    top_nodes: list[UID] | list[int],
    bottom_nodes: list[UID] | list[int],
    ax: Optional[matplotlib.axes.Axes] = None,
    return_pos: typing.Literal[False] = False,
    label_nodes: bool = False,
) -> matplotlib.axes.Axes: ...
...
def draw_graph_in_out_center_circle(
    ...
    other_nodes = [
        node
        for node in graph.nodes
        if node not in top_nodes and node not in bottom_nodes
    ]
...
def position_3d_nodes(x: list, y: list, z: list):
    ...
    nodes_xyz = {node: np.array([x_pos[i], 0, 0]) for i, node in enumerate(x)}
    nodes_xyz.update({node: np.array([0, y_pos[i], 0]) for i, node in enumerate(y)})
    nodes_xyz.update({node: np.array([0, 0, z_pos[i]]) for i, node in enumerate(z)})
    return nodes_xyz
...
def plot_xyz(
    ...
    if pos is None:
        if z is None:
            z = [node for node in graph.nodes if node not in x and node not in y]
        # sort the nodes
        axis = [x, y, z]
        # sort the nodes
        for index in [0, 2, 1]:  # sort x, then z, then y
            ...
            if sort_ == "input_clustering":
                refs = [
                    ...
                    axis[2],
                    axis[0],
                ]  # will be modified, different from [x,y,z]
                axis[index] = nx_utils.sort_nodes(
                    graph, axis[index], sort_, ref=refs[index]
                )
...
def draw_3d(
    ...
    assert isinstance(ax, Axes3D)  # needed for type hinting
