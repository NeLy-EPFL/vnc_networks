...
def confusion_matrix_sign_assymetry_mdn_to_mn():
    ...
    eff_weight_abs = VNC.get_cmatrix(type_='norm')
    eff_weight_abs.absolute()
    eff_weight_abs.within_power_n(2) # effective connection strength to each MN if all connections are positive
    eff_weight_rel = VNC.get_cmatrix(type_='norm')
    eff_weight_rel.within_power_n(2) # effective connection strength to each MN keeping origin signs
    ...
    for i in range(3):
        for j in range(3):
            ...
            pos_matrix = mat.get_positive_matrix()
            neg_matrix = mat.get_negative_matrix()
            confusion_matrix[i,j] = abs(pos - neg)/(pos + neg)
    # Plot the confusion matrix
    ax = matrix_design.imshow(
        confusion_matrix,
        ylabel='MDN subdivision',
        ...
        xlabel='leg motor neuron',
        col_labels=['f', 'm', 'h'],
        title='Neg Confusion matrix of MDN to MN connections',
        cmap=params.grey_heatmap,
        )
    plt.savefig(os.path.join(FOLDER, f'Fig3_sign_imbalance_neg.pdf'))
    plt.close()
