...
of the type of neurons, given by the attribute chosen (default 'class_1').
...
FOLDER_NAME = os.path.join("MDN_specific","Exploration_Graphs")
...
def fig1b(n_hops: int = 2):
    ...
    VNC = (
        VNC.get_connections_with_only_traced_neurons()
...
def fig1d():
    ...
    # saving
    plt.savefig(os.path.join(FOLDER, "Fig1d.pdf"))
    plt.close()


def fig1e(attribute: NeuronAttribute = "class_1"):
    print("> Fig 1e")
    ...
    for i in range(3):
        set_neurons = list_down_neurons[i].intersection(list_down_neurons[(i + 1) % 3])
        VNC.draw_bar_plot(set_neurons, attribute, ax=axs[i])
        axs[i].set_title(f"overlap downstream of MDNs|T{i+1} & MDNs|T{(i+1)%3 + 1}")
    plt.tight_layout()
...
def fig1f(syn_thresh: int = None, label_nodes: bool = False):
    ...
    for j, side in enumerate(["L", "R"]):
        for i, leg in enumerate(["f", "m", "h"]):
            ...
            target_neurons = list(set(target_neurons).intersection(leg_motor_neurons))
...
def fig1g(
    attribute: str = "class_1",
    syn_thresh: Optional[int] = None,
):
    print("> Fig 1g")
...
if __name__ == "__main__":
    ...
    # fig1c()
    # fig1d()
    # fig1e(attribute='class_1')
    # fig1e(attribute='class_2')
    # fig1e(attribute='neuropil')
    # fig1e(attribute='target')
    # fig1e(attribute='side')
    # fig1f()
    # fig1f(syn_thresh=40, label_nodes=True)
    # fig1g(attribute='target')
    # fig1g(attribute='target',syn_thresh=40)
    # Example target:
