...
def focus_strongest_inhibitors_t3_effective(
    ...
    _, axs = plt.subplots(
        1,
        3,
        figsize=(3*params.FIGSIZE[0], params.FIGSIZE[1])
        )
...
def focus_specific_neurons_effective(
    ...
    plt.tight_layout()
    title = f'specific_neurons_inh_effective'
    plt.savefig(os.path.join(FOLDER, title +'.pdf'))
    plt.close()
...
def control_t3_motor_neuron_clusters():
    ...
    plt.savefig(os.path.join(
        FOLDER,
        'control_clustered_t3_motor_neurons_matrix.pdf')
        )
    plt.close()
...
def compare_mn_clusters_control_vs_mdnl2():
    ...
    mdn_induced_clusters = read_clusters_from_file(
        os.path.join(FOLDER, 'clusters.csv')
        )
    ...
    for i, c1 in enumerate(motor_mdn_clusters):
        for j, c2 in enumerate(motor_control_clusters):
            print(f'Overlap between clusters {i} (lenght({len(c1)})) and {j} (length {len(c2)}): {len(set(c1) & set(c2))}')
