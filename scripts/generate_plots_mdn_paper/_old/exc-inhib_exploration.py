...
def simple_count_in_neuropil(
    ...
    neurons_used.extend(  # intereurons
        VNC.get_neurons_in_neuropil(neuropil=neuropil, side=side + "HS")
    )
    target = list(
        mns_helper.get_leg_motor_neurons(data=VNC, leg=leg, side=side + "HS")
    )  # motor neurons
    ...
    print(
        f"Number of {leg} motor neurons reached from MDN in {neuropil} {side}: {len(list_down)}"
    )
...
def fig2a():
    ...
    args_list = []
    result_indices = [(i, j) for i in range(nb_entries) for j in range(nb_entries)]
